...
SELECT il.AD_Client_ID, il.AD_Org_ID,
    ...
    dt.IsFiscalDocument, 
    MIN(CASE WHEN il.DocStatus IN('CO', 'CL') THEN i.DocumentNo ELSE '' END) AS BeginingDocumentNo,
    MAX(CASE WHEN il.DocStatus IN('CO', 'CL') THEN i.DocumentNo ELSE '' END) AS EndingDocumentNo,
    MIN(CASE WHEN il.DocStatus IN('CO', 'CL') THEN i.ControlNo ELSE '' END) AS BeginingControlNo,

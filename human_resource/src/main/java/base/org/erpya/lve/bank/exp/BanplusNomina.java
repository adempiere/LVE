 ...
 */
public class BanplusNomina extends LVEPaymentExportList {
	...
	public int exportToFile(List<MPaySelectionCheck> checks, File file, StringBuffer error) {
		...
		try {
			...
			//	Debt Note
			header.append(organizationType)		//  Organization Type
				.append(orgTaxId)				//	Org Tax
				.append(SEPARATOR)
				.append(bankAccountNo) 			//	Bank Client No
				.append(SEPARATOR)
				.append(checks.size())      	//  Qty 
				.append(SEPARATOR)
				.append(totalAmtAsString)		//	Total Amount
				.append(SEPARATOR)
				.append(payDate);				//	Pay Date
			//	Open File
					...
					.forEach(payselectionCheck -> {
						...
						if(bpAccount != null) {
							...
							if(!Util.isEmpty(bPTaxId)){
								...
								bPTaxId = leftPadding(bPTaxId, 15, "", true);
							} else {
							...
							amountAsString = leftPadding(amountAsString, 18, "", true);
							...
							line.append(CRLF)			 //	New Line
								...
								.append(SEPARATOR)
								.append(bPTaxId)		 //  BP Bank Account
								.append(SEPARATOR)
